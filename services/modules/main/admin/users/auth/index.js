export function Authorisation(s) {
    ...
    if( Result.success ) {
        ...
        if( App.User.err.code !== "" ) {
            open_modal('err_alert', '#?w=200px&h=100px', App.User.err.msg, 'popup_block_err');
        } else {
            //Успешная авторизация
            open_modal('err_alert', '#?w=200px&h=100px', App.User.fio);
            //Скрываем кнопку авторизации, размещаем ФИО, открываем кнопку выхода
            $("#UserAuth").addClass("dn");
            $("#UserFIO").html(App.User.fio);
            $("#UserProp").removeClass("dn");
        }
    } else {
        open_modal('err_alert', '#?w=200px&h=100px', Result.User.msg);       
    }
...
export function getAuthForm(prop) {
    
    let _Params = {
        url: "/services/modules/main/admin/users/auth/form.php",
        async: false,
        data: {},
        selector: "#UserAuthForm",
        replace: true
    };
    ...
    if( _Params.url != "" ) {
        Result = getContent_putNode(_Params); //Получаем HTML код с сервера
        
        //Если успешно получили
        if( Result.success ) {
            //Открываем модальное окно с формой ввода логина-пароля
            ContentToModal(_Params.selector, '#?w=300px&h=200px');
        } else {
            ContentToModal(_Params.selector, '#?w=300px&h=200px', 'popup_block_err');
        }
